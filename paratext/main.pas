...
uses
  pollthread, Registry, books, utilities, santafefocus;
...
begin
  ...
  registry.OpenKey('\Software\Bibledit\Paratext', True);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          paratext := BookIdToParatext (StrToInt (reference[0]));
          paratext := paratext + ' ' + reference[1] + ':' + reference[2];
          WriteSantaFeFocusScriptureReference (paratext);
          LabelParatext.Caption := 'WriteSantaFeFocusScripturereference (' + paratext + ')';
        end;
        reference.Free;
        thread := TPollThread.Create(false);
      end;
