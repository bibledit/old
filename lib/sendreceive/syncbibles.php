...
if ($server_checksum === false) {
  $database_logs->log (gettext("Failure getting Bibles checksum"), Filter_Roles::TRANSLATOR_LEVEL);
  die;
}

if ($client_checksum == $server_checksum) {
  $database_logs->log (gettext("The Bibles are up to date"), Filter_Roles::TRANSLATOR_LEVEL);
  die;
}
...
if ($server_bibles === false) {
  $database_logs->log (gettext("Failure getting list of Bibles"), Filter_Roles::TRANSLATOR_LEVEL);
  die;
}
...
if ($server_checksum != $message_checksum) {
  $database_logs->log (gettext("Checksum error while receiving list of Bibles from server:"), Filter_Roles::TRANSLATOR_LEVEL);
  die;
}
$database_logs->log (gettext("Bibles") . ": " . implode (", ", $server_bibles), Filter_Roles::TRANSLATOR_LEVEL);
...
foreach ($bibles as $bible) {
  ...
  $database_logs->log (gettext("Deleting Bible because the server did not grant access to it") . ": " . $bible, Filter_Roles::TRANSLATOR_LEVEL);
}
...
foreach ($server_bibles as $bible) {
  ...
  $server_checksum = Sync_Logic::post ($post, $url);
  if ($server_checksum === false) {
    $database_logs->log (gettext("Failure getting Bible checksum"), Filter_Roles::TRANSLATOR_LEVEL);
    continue;
  }
  if ($client_checksum == $server_checksum) {
    $database_logs->log (gettext("The Bible is up to date") . ": $bible", Filter_Roles::TRANSLATOR_LEVEL);
    continue;
  }
  ...
  if ($server_books === false) {
    $database_logs->log (gettext("Failure getting Bible books"), Filter_Roles::TRANSLATOR_LEVEL);
    continue;
  }
  ...
  if ($server_checksum != $message_checksum) {
    $database_logs->log (gettext("Checksum error while receiving list of books from server"), Filter_Roles::TRANSLATOR_LEVEL);
    continue;
  }
  ...
  foreach ($client_books as $book) {
    ...
    $database_logs->log (gettext("Deleting book because the server does not have it") . ": $bible $book_name" , Filter_Roles::TRANSLATOR_LEVEL);
  }
  ...
  foreach ($server_books as $book) {
    ...
    $server_checksum = Sync_Logic::post ($post, $url);
    if ($server_checksum === false) {
      $database_logs->log (gettext("Failure getting book checksum"), Filter_Roles::TRANSLATOR_LEVEL);
      continue;
    }
    if ($client_checksum == $server_checksum) {
      continue;
    ...
    if ($server_chapters === false) {
      ...
      $database_logs->log (gettext("Failure getting list of chapters:") . " $bible $bookname", Filter_Roles::TRANSLATOR_LEVEL);
      continue;
    }
    ...
    if ($server_checksum != $message_checksum) {
      $database_logs->log (gettext("Checksum error while receiving list of chapters from server"), Filter_Roles::TRANSLATOR_LEVEL);
      continue;
    }
    ...
    foreach ($client_chapters as $chapter) {
      ...
      $database_logs->log (gettext("Deleting chapter because the server does not have it") . ": $bible $book_name $chapter" , Filter_Roles::TRANSLATOR_LEVEL);
    }
    ...
    foreach ($server_chapters as $chapter) {
      ...
      $server_checksum = Sync_Logic::post ($post, $url);
      if ($server_checksum === false) {
        $database_logs->log (gettext("Failure getting checksum:") . " $bible $book_name $chapter", Filter_Roles::TRANSLATOR_LEVEL);
        continue;
      }
      if ($client_checksum == $server_checksum) {
        continue;
      ...
      $database_logs->log (gettext("Getting chapter:") . " $bible $book_name $chapter", Filter_Roles::TRANSLATOR_LEVEL);
      $post = array (
        "b" => $bible,
        "bk" => $book,
        "c" => $chapter,
      ...
      if ($server_usfm === false) {
        $database_logs->log (gettext("Failure getting chapter:") . " $bible $book_name $chapter", Filter_Roles::TRANSLATOR_LEVEL);
        continue;
      }
      ...
      if (!Checksum_Logic::get ($server_usfm) == $checksum) {
        $database_logs->log (gettext("Checksum error while receiving chapter from server:") . " $bible $book_name $chapter", Filter_Roles::TRANSLATOR_LEVEL);
        continue;
      }
      ...
      $database_logs->log (gettext("Merging changes on server and client:") . " $bible $book_name $chapter", Filter_Roles::TRANSLATOR_LEVEL);
