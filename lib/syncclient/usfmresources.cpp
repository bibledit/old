...
int syncclient_usfmresources_watchdog = 0;


void syncclient_usfmresources_done ()
{
  syncclient_usfmresources_watchdog = 0;
}


string syncclient_usfmresources_text ()
{
  return translate("USFM resources") + ": ";
}


string syncclient_usfmresources_sendreceive_text ()
{
  return syncclient_usfmresources_text () + translate ("Synchronizing");
}


string syncclient_usfmresources_up_to_date_text ()
{
  return syncclient_usfmresources_text () + translate ("Up to date");
}


void syncclient_usfmresources ()
{
  if (syncclient_usfmresources_watchdog) {
    int time = filter_date_seconds_since_epoch ();
    if (time < (syncclient_usfmresources_watchdog + 900)) {
      Database_Logs::log (syncclient_usfmresources_text () + translate("Still busy"), Filter_Roles::translator ());
      return;
    }
    Database_Logs::log (syncclient_usfmresources_text () + translate("Watchdog timeout"), Filter_Roles::translator ());
  }
  syncclient_usfmresources_kick_watchdog ();
  ...
  Database_Logs::log (syncclient_usfmresources_sendreceive_text (), Filter_Roles::translator ());
  ...
  if (!error.empty ()) {
    Database_Logs::log (syncclient_usfmresources_text () + "Failure getting total checksum: " + error, Filter_Roles::translator ());
    syncclient_usfmresources_done ();
    return;
  }
  string checksum = Sync_Logic::usfm_resources_checksum ();
  if (response == checksum) {
    Database_Logs::log (syncclient_usfmresources_up_to_date_text (), Filter_Roles::translator ());
    syncclient_usfmresources_done ();
    return;
  }
  ...
  response = sync_logic.post (post, url, error);
  if (!error.empty ()) {
    Database_Logs::log (syncclient_usfmresources_text () + "Failure getting resources: " + error, Filter_Roles::translator ());
    syncclient_usfmresources_done ();
    return;
  }
  ...
  for (auto & resource : server_resources) {
    ...
    response = sync_logic.post (post, url, error);
    if (!error.empty ()) {
      Database_Logs::log (syncclient_usfmresources_text () + "Failure getting checksum of resource: " + error, Filter_Roles::translator ());
      syncclient_usfmresources_done ();
      return;
    }
    ...
    response = sync_logic.post (post, url, error);
    if (!error.empty ()) {
      Database_Logs::log (syncclient_usfmresources_text () + "Failure getting books of resource: " + error, Filter_Roles::translator ());
      syncclient_usfmresources_done ();
      return;
    }
    ...
    for (auto & book : server_books) {
      ...
      response = sync_logic.post (post, url, error);
      if (!error.empty ()) {
        Database_Logs::log (syncclient_usfmresources_text () + "Failure getting checksum of resource book: " + error, Filter_Roles::translator ());
        syncclient_usfmresources_done ();
        return;
      }
      ...
      Database_Logs::log (syncclient_usfmresources_text () + "Synchronizing " + resource + " " + bookname, Filter_Roles::translator ());
      ...
      response = sync_logic.post (post, url, error);
      if (!error.empty ()) {
        Database_Logs::log (syncclient_usfmresources_text () + "Failure getting chapters of resource book: " + error, Filter_Roles::translator ());
        syncclient_usfmresources_done ();
        return;
      }
      ...
      for (auto & chapter : server_chapters) {
        ...
        response = sync_logic.post (post, url, error);
        if (!error.empty ()) {
          Database_Logs::log (syncclient_usfmresources_text () + "Failure getting checksum of resource chapter: " + error, Filter_Roles::translator ());
          syncclient_usfmresources_done ();
          return;
        }
        ...
        response = sync_logic.post (post, url, error);
        if (!error.empty ()) {
          Database_Logs::log (syncclient_usfmresources_text () + "Failure downloading resource chapter: " + error, Filter_Roles::translator ());
          syncclient_usfmresources_done ();
          return;
        }
  ...
  // Done.
  Database_Logs::log (syncclient_usfmresources_text () + "Now up to date", Filter_Roles::translator ());
  syncclient_usfmresources_done ();
}


void syncclient_usfmresources_kick_watchdog ()
{
  syncclient_usfmresources_watchdog = filter_date_seconds_since_epoch ();
}
