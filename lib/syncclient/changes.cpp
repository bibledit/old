...
int syncclient_changes_watchdog = 0;


void sync_client_changes_done ()
{
  syncclient_changes_watchdog = 0;
}


string syncclient_changes_text ()
{
  return translate("Changes") + ": ";
}


string syncclient_changes_sendreceive_text ()
{
  return syncclient_changes_text () + translate ("Send/receive");
}


string syncclient_changes_up_to_date_text ()
{
  return syncclient_changes_text () + translate ("Up to date");
}


void syncclient_changes ()
{
  if (syncclient_changes_watchdog) {
    int time = filter_date_seconds_since_epoch ();
    if (time < (syncclient_changes_watchdog + 900)) {
      Database_Logs::log (syncclient_changes_text () + translate("Still busy"), Filter_Roles::translator ());
      return;
    }
    Database_Logs::log (syncclient_changes_text () + translate("Watchdog timeout"), Filter_Roles::translator ());
  }
  syncclient_changes_kick_watchdog ();

  
  Database_Logs::log (syncclient_changes_sendreceive_text (), Filter_Roles::translator ());
  ...
  if (!database_modifications.healthy ()) {
    Database_Logs::log (syncclient_changes_text () + translate("Recreate damaged modifications database"), Filter_Roles::translator ());
  ...
  if (iresponse < Filter_Roles::guest () || iresponse > Filter_Roles::admin ()) {
    Database_Logs::log (syncclient_changes_text () + translate("Failure to initiate connection"), Filter_Roles::translator ());
    sync_client_changes_done ();
    return;
  }
  ...
  if (!ids.empty ()) Database_Logs::log (syncclient_changes_text () + "Sending removed notifications: " + convert_to_string (ids.size()), Filter_Roles::translator ());
  for (auto & id : ids) {
    ...
    if (!error.empty ()) {
      ...
      Database_Logs::log (syncclient_changes_text () + "Failure sending removed notification: " + error, Filter_Roles::translator ());
    }
    else {
  ...
  if (communication_errors) {
    Database_Logs::log (syncclient_changes_text () + translate("Not downloading change notifications due to communication error"), Filter_Roles::translator ());
    sync_client_changes_done ();
    return;
  }
  ...
  response = sync_logic.post (post, url, error);
  if (!error.empty ()) {
    Database_Logs::log (syncclient_changes_text () + "Failure receiving checksum: " + error, Filter_Roles::translator ());
    sync_client_changes_done ();
    return;
  }
  server_checksum = response;
  if (client_checksum == server_checksum) {
    Database_Logs::log (syncclient_changes_up_to_date_text (), Filter_Roles::translator ());
    sync_client_changes_done ();
    return;
  }
  ...
  response = sync_logic.post (post, url, error);
  if (!error.empty ()) {
    Database_Logs::log (syncclient_changes_text () + "Failure receiving identifiers: " + error, Filter_Roles::translator ());
    sync_client_changes_done ();
    return;
  }
  {
  ...
  for (auto & id : remove_identifiers) {
    ...
    Database_Logs::log (syncclient_changes_text () + "Removing notification: " + convert_to_string (id), Filter_Roles::translator ());
  }
  ...
  for (auto & id : download_identifiers) {
    syncclient_changes_kick_watchdog ();
    Database_Logs::log (syncclient_changes_text () + "Downloading notification: " + convert_to_string (id), Filter_Roles::translator ());
    post ["a"] = convert_to_string (Sync_Logic::changes_get_modification);
    post ["i"] = convert_to_string (id);
    response = sync_logic.post (post, url, error);
    if (!error.empty ()) {
      Database_Logs::log (syncclient_changes_text () + "Failure downloading notification: " + error, Filter_Roles::translator ());
    }
    else {
  ...
  // Done.
  Database_Logs::log (syncclient_changes_text () + "Ready", Filter_Roles::translator ());
  sync_client_changes_done ();
}


void syncclient_changes_kick_watchdog ()
{
  syncclient_changes_watchdog = filter_date_seconds_since_epoch ();
}
