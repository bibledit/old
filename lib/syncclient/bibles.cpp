...
int syncclient_bibles_watchdog = 0;


void sync_client_bibles_done ()
{
  syncclient_bibles_watchdog = 0;
}


string syncclient_bibles_text ()
{
  return translate("Bibles") + ": ";
}


string syncclient_bibles_sendreceive_text ()
{
  return syncclient_bibles_text () + translate ("Send/receive");
}


string syncclient_bibles_up_to_date_text ()
{
  return syncclient_bibles_text () + translate ("Up to date");
}


void syncclient_bibles ()
{
  if (syncclient_bibles_watchdog) {
    int time = filter_date_seconds_since_epoch ();
    if (time < (syncclient_bibles_watchdog + 900)) {
      Database_Logs::log (syncclient_bibles_text () + translate("Still busy"), Filter_Roles::translator ());
      return;
    }
    Database_Logs::log (syncclient_bibles_text () + translate("Watchdog timeout"), Filter_Roles::translator ());
  }
  syncclient_bibles_kick_watchdog ();

  
  Database_Logs::log (syncclient_bibles_sendreceive_text (), Filter_Roles::translator ());
  ...
  if (iresponse < Filter_Roles::guest () || iresponse > Filter_Roles::admin ()) {
    Database_Logs::log (syncclient_bibles_text () + translate("Failure to initiate connection"), Filter_Roles::translator ());
    sync_client_bibles_done ();
    return;
  }
  ...
  for (string bible : bibles) {
    ...
    for (int book : books) {
      ...
      for (int chapter : chapters) {
        ...
        Database_Logs::log (syncclient_bibles_text () + translate("Sending to server") + ": " + bible + " " + bookname + " " + convert_to_string (chapter), Filter_Roles::translator ());
        ...
        if ((newusfm != oldusfm) && (newusfm != "")) {
          ...
          if (!error.empty ()) {
            ...
            Database_Logs::log (syncclient_bibles_text () + "Failure sending chapter: " + error, Filter_Roles::translator ());
            ...
            if (!response.empty ()) {
              Database_Logs::log (syncclient_bibles_text () + "Bibledit Cloud says: " + response, Filter_Roles::translator ());
            }
          }
        }
      }
  ...
  if (communication_errors) {
    Database_Logs::log (syncclient_bibles_text () + "Not downloading changes from the server due to communication errors", Filter_Roles::translator ());
    sync_client_bibles_done ();
    return;
  }
  ...
  if (!error.empty ()) {
    Database_Logs::log (syncclient_bibles_text () + translate("Failure getting total checksum") + ": " + error, Filter_Roles::translator ());
    sync_client_bibles_done ();
    return;
  }
  if (client_checksum == server_checksum) {
    Database_Logs::log (syncclient_bibles_up_to_date_text (), Filter_Roles::translator ());
    sync_client_bibles_done ();
    return;
  }
  ...
  if (!error.empty () || v_server_bibles.empty ()) {
    Database_Logs::log (syncclient_bibles_text () + translate("Failure getting list of Bibles"), Filter_Roles::translator ());
    sync_client_bibles_done ();
    return;
  }
  ...
  if (server_checksum != message_checksum) {
    Database_Logs::log (syncclient_bibles_text () + translate("Checksum error while receiving list of Bibles"), Filter_Roles::translator ());
    sync_client_bibles_done ();
    return;
  }
  Database_Logs::log (syncclient_bibles_text () + filter_string_implode (v_server_bibles, ", "), Filter_Roles::translator ());
  ...
  for (string bible : bibles) {
    ...
    Database_Logs::log (syncclient_bibles_text () + translate("Deleting Bible because the server did not grant access to it") + ": " + bible, Filter_Roles::translator ());
  }
  ...
  for (string bible : v_server_bibles) {
    ...
    if (!error.empty () || client_checksum.empty ()) {
      Database_Logs::log (syncclient_bibles_text () + translate("Failure getting Bible checksum") + ": " + error, Filter_Roles::translator ());
      continue;
    }
    if (client_checksum == server_checksum) {
      Database_Logs::log (syncclient_bibles_text () + translate("Bible up to date") + ": " + bible, Filter_Roles::translator ());
      continue;
    }
    ...
    if (!error.empty () || server_books.empty ()) {
      Database_Logs::log (syncclient_bibles_text () + translate("Failure getting books") + ": " + error, Filter_Roles::translator ());
      continue;
    }
    ...
    if (server_checksum != message_checksum) {
      Database_Logs::log (syncclient_bibles_text () + translate("Checksum error while receiving list of books from server"), Filter_Roles::translator ());
      continue;
    }
    ...
    for (auto & book : client_books) {
      ...
      Database_Logs::log (syncclient_bibles_text () + translate("Deleting book because the server does not have it") + ": " + bible + " " + book_name , Filter_Roles::translator ());
    }
    ...
    for (auto & book : i_server_books) {
      ...
      string server_checksum = sync_logic.post (post, url, error);
      if (!error.empty ()) {
        Database_Logs::log (syncclient_bibles_text () + translate("Failure getting book checksum") + ": " + error, Filter_Roles::translator ());
        continue;
      }
      if (client_checksum == server_checksum) {
        continue;
      ...
      if (!error.empty () || server_chapters.empty ()) {
        Database_Logs::log (syncclient_bibles_text () + translate("Failure getting list of chapters:") + " " + bible + " " + book_name, Filter_Roles::translator ());
        continue;
      }
      ...
      if (server_checksum != message_checksum) {
        Database_Logs::log (syncclient_bibles_text () + translate("Checksum error while receiving list of chapters"), Filter_Roles::translator ());
        continue;
      }
      ...
      for (auto & chapter : client_chapters) {
        ...
        Database_Logs::log (syncclient_bibles_text () + translate("Deleting chapter because the server does not have it") + ": " + bible + " " + book_name + " " + convert_to_string (chapter), Filter_Roles::translator ());
      }
      ...
      for (auto & chapter : i_server_chapters) {
        ...
        if (!error.empty () || server_checksum.empty ()) {
          Database_Logs::log (syncclient_bibles_text () + translate("Failure getting checksum:") + " " + bible + " " + book_name + " " + convert_to_string (chapter), Filter_Roles::translator ());
          continue;
        }
        if (client_checksum == server_checksum) {
          continue;
        ...
        Database_Logs::log (syncclient_bibles_text () + translate("Getting chapter:") + " " + bible + " " + book_name + " " + convert_to_string (chapter), Filter_Roles::translator ());
        ...
        if (!error.empty () || server_usfm.empty ()) {
          Database_Logs::log (syncclient_bibles_text () + translate("Failure getting chapter:") + " " + bible + " " + book_name + " " + convert_to_string (chapter), Filter_Roles::translator ());
          continue;
        }
        ...
        if (Checksum_Logic::get (server_usfm) != checksum) {
          Database_Logs::log (syncclient_bibles_text () + translate("Checksum error while receiving chapter from server:") + " " + bible + " " + book_name + " " + convert_to_string (chapter), Filter_Roles::translator ());
          continue;
        }
        ...
        Database_Logs::log (syncclient_bibles_text () + translate("Merging changes on server and client") + " " + bible + " " + book_name + " " + convert_to_string (chapter), Filter_Roles::translator ());
  ...
  // Done.
  Database_Logs::log (syncclient_bibles_text () + "Ready", Filter_Roles::translator ());
  sync_client_bibles_done ();
}


void syncclient_bibles_kick_watchdog ()
{
  syncclient_bibles_watchdog = filter_date_seconds_since_epoch ();
}
