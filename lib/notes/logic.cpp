...
private function emailUsers (identifier, label, users, postpone)
{
  ...
  mailto = "mailto:" . database_config_general.getSiteMailAddress () . "?subject=(CNIDidentifier)";
  ...
  foreach (users as user) {
    if (!config_logic_enabled ()) database_mail.send (user, "label | passages | summary | (CNIDidentifier)", contents, timestamp);
  }
}
...
public function handleEmailComment (from, subject, body)
{
  ...
  if (database_config_user.getUserNotifyMeOfMyPosts (username)) {
    ...
    database_mail.send (username, "subject [CNIDidentifier]", body);
  }
  // Log operation.
