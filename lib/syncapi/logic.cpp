...
SyncAPI_Logic::SyncAPI_Logic (void * webserver_request_in)
{
  webserver_request = webserver_request_in;
}


SyncAPI_Logic::~SyncAPI_Logic ()
{
}


bool SyncAPI_Logic::credentials_okay ()
{
...
string SyncAPI_Logic::checksum (const vector <int> & identifiers)
{
...
vector <SyncAPI_Logic_Range> SyncAPI_Logic::create_range (int start, int end)
{
  int range = end - start;
  range = round (range / 10);
  vector <SyncAPI_Logic_Range> ranges;
...
string SyncAPI_Logic::post (map <string, string> & post, const string& url, string & error, bool burst)
{
  error.clear ();
...
string SyncAPI_Logic::settings_checksum (const vector <string> & bibles)
{
  Webserver_Request * request = (Webserver_Request *) webserver_request;
  string checksum;
...
string SyncAPI_Logic::usfm_resources_checksum ()
{
...
string SyncAPI_Logic::usfm_resource_checksum (const string& name)
{
  vector <string> vchecksum;
  Database_UsfmResources database_usfmresources = Database_UsfmResources ();
...
string SyncAPI_Logic::usfm_resource_book_checksum (const string& name, int book)
{
  vector <string> vchecksum;
  Database_UsfmResources database_usfmresources = Database_UsfmResources ();
...
string SyncAPI_Logic::usfm_resource_chapter_checksum (const string& name, int book, int chapter)
{
  Database_UsfmResources database_usfmresources = Database_UsfmResources ();
...
string SyncAPI_Logic::offline_resources_checksum ()
{
  string checksum;
...
string SyncAPI_Logic::offline_resource_checksum (const string& name)
{
...
string SyncAPI_Logic::offline_resource_file_checksum (const string& name, const string& file)
{
  Database_OfflineResources database_offlineresources = Database_OfflineResources ();
...
string SyncAPI_Logic::changes_checksum (const string & username)
{
