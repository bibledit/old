...
function handleSelection (selector) // Todo integrate into other code where styles are applied.
{
  ...
  quill.formatLine (10, 10, 'align', 'center');
}
  ...
  $.ajax ({
    ...
    success: function (response) {
      ...
      if (response !== false) {
        ...
        //$ ("#editor-container").append (response);
        // Create the editor to have it load the DOM's content.
        // Note: When reloading the chapter, it should have the undo operations, which is not the case now.
        // quill = new Quill ('#editor-container', { modules: { toolbar: '#toolbar-container' }, theme: 'snow' });
