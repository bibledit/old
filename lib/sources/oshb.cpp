...
/* Todo
void sources_oshb_parse_w_element (Database_oshb * database_oshb, int book, int chapter, int verse, xml_node node)
{
...
void sources_oshb_parse_unhandled_node (int book, int chapter, int verse, xml_node node)
{
...
void sources_oshb_parse ()
{
  ...
  for (size_t bk = 0; bk < books.size (); bk++) {
    ...
    string file = "sources/oshb/" + books[bk] + ".xml";
    cout << file << endl;
    ...
    for (xml_node chapter_node : div_book_node.children()) {
      for (xml_node verse_node : chapter_node.children ()) {
        ...
        for (xml_node node : verse_node.children ()) {
          ...
          if (node_name == "w") {
            sources_oshb_parse_w_element (&database_oshb, book, chapter, verse, node);
          }
            ...
            for (xml_node variant_node : node.children ()) {
              ...
              if (node_name == "catchWord") {
                ...
                for (xml_node w_node : variant_node.children ()) {
                  ...
                  sources_oshb_parse_w_element (&database_oshb, book, chapter, verse, w_node);
                }
              } else {
                sources_oshb_parse_unhandled_node (book, chapter, verse, node);
              }
            }
          }
