...
string personalize_index (void * webserver_request)
{
  ...
  if (request->query.count (activevisualeditors)) {
    ...
    if (editors.empty ()) {
      ...
      for (int i = 0; i < 3; i++) {
        dialog_list.add_row (bible_logic_editor_settings_text (true, i), activevisualeditors, convert_to_string (i));
      }
  ...
  editors = bible_logic_editor_settings_text (true, request->database_config_user ()->getEnabledVisualEditors ());
  ...
  if (request->query.count (activeusfmeditors)) {
    ...
    if (editors.empty ()) {
      Dialog_List dialog_list = Dialog_List ("index", translate("Which visual Bible editors to enable?"), "", "");
      for (int i = 0; i < 3; i++) {
        dialog_list.add_row (bible_logic_editor_settings_text (false, i), activeusfmeditors, convert_to_string (i));
      }
      page += dialog_list.run ();
      return page;
    } else {
  ...
  editors = bible_logic_editor_settings_text (false, request->database_config_user ()->getEnabledUsfmEditors ());
