...
function oneverseEditorLoadVerse ()
{
  if ((oneverseNavigationBook != oneverseBook) || (oneverseNavigationChapter != oneverseChapter) || (oneverseNavigationVerse != oneverseVerse) || oneverseReload) {
    ...
    oneverseIdChapter = 0;
    $ ("#oneeditor").focus;
    oneverseCaretPosition = getCaretPosition (); // Todo can go out probably.
    $.ajax ({
      url: "load",
      type: "GET",
      data: { bible: oneverseBible, book: oneverseBook, chapter: oneverseChapter, verse: oneverseVerse, part: "verse" },
      success: function (response) {
        ...
        if (response !== false) {
          ...
          if (oneverseReload) {
            positionCaret (oneverseCaretPosition);
          }
          oneverseReload = false;
        } else {
...
function oneverseEditorSaveVerse (sync)
{
  ...
  $.ajax ({
    ...
    data: { bible: oneverseBible, book: oneverseBook, chapter: oneverseChapter, verse: oneverseVerse, html: html, checksum: checksum },
