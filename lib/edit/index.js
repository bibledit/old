...
function editorGetHtml () // Todo
{
  ...
  // * the last class name.
  html = html.split (' focusedverse').join ('');
  // Todo for displaying active style, leave 'focusedverse' out, but this is elsewhere.
  return html;
}
...
function editorScrollVerseIntoView () // Todo ensure it does not markup notes or what.
{
  var iterVerse = 0;
  $ ("#editor span").each (function (index) {
    var element = $(this);
