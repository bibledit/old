...
function editorScrollVerseIntoView ()
{
  ...
  $ ("#editor > p span").each (function (index) {
    ...
    if (element.hasClass ("v")) {
      ...
      iterVerse = element[0].textContent; // Todo innerText is non-standard, use textContent: Update anywhere, and test it.
      if (iterVerse == editorNavigationVerse) {
        if (navigated == false) {
          ...
          console.log ("scrollTo"); console.log (scrollTo); // Todo
          ...
          if ((scrollTo < lowerBoundary) || (scrollTo > upperBoundary)) {
            $ ("body,html").animate ({ scrollTop: scrollTo }, 500); // Todo fails on Firefox.
          }
          navigated = true;
        }
      }
