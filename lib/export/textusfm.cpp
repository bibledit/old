...
void export_text_usfm_book (string bible, int book, bool force) // Todo
{
  ...
  if (!file_exists (textDirectory)) filter_url_mkdir (textDirectory);
  ...
  // Basic USFM.
  if (file_exists (usfmFilename)) filter_url_unlink (usfmFilename);
