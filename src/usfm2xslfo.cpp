...
void Usfm2XslFo::convert_from_usfm_to_xslfo ()
{
  // Cancel?
  if (cancel) return;
        ...
        if (marker_position == 0) {
          ...
          if (stylepointer) {
            switch (stylepointer->type) {
              ...
              case u2xtColumnBalancer:
              {
                output_column_balancer (usfm_line, fo_block_style, fo_inline_style, marker_length);
                break;
              }
              case u2xtElastic:
              {
  ...
  // Save the xslfo text to disk.
  g_file_set_contents (xslfofile ().c_str(), (const gchar *) buffer->content, -1, NULL);
}
...
{
  for (unsigned int i = 0; i < styles.size (); i++) {
    if (marker == styles[i].marker) return &styles[i];
  }
  return NULL;
}
...
void Usfm2XslFo::open_fo_inline (Usfm2XslFoStyle * style, Usfm2XslFoStyle * block)
{
  ...
  // Italic
  s.clear ();
  if (style->italic == ON) s = "italic";
  else if (style->italic == OFF) s =("normal");
  else if (style->italic == INHERIT) s = "inherit";
  else if (style->italic == TOGGLE) {
    if (block) {
      if (block->italic == ON) s = "normal";
      else s = "italic";
    }
  }
  if (!s.empty ()) {
  ...
  // Bold
  s.clear ();
  if (style->bold == ON) s = "bold";
  else if (style->bold == OFF) s =("normal");
  else if (style->bold == INHERIT) s = "inherit";
  else if (style->bold == TOGGLE) {
    if (block) {
      if (block->bold == ON) s = "normal";
      else s = "bold";
    }
  }
  if (!s.empty ()) {
  ...
  // Underline
  s.clear ();
  if (style->underline == ON) s = "underline";
  else if (style->underline == OFF) s =("none");
  else if (style->underline == INHERIT) s = "inherit";
  else if (style->underline == TOGGLE) {
    if (block) {
      if (block->underline == ON) s = "none";
      else s = "underline";
    }
  }
  if (!s.empty ()) {
  ...
  // Smallcaps
  s.clear ();
  if (style->smallcaps == ON) s = "small-caps";
  else if (style->smallcaps == OFF) s =("normal");
  else if (style->smallcaps == INHERIT) s = "inherit";
  else if (style->smallcaps == TOGGLE) {
    if (block) {
      if (block->smallcaps == ON) s = "normal";
      else s = "small-caps";
    }
  }
  if (!s.empty ()) {
...
// Else it stores the data needed for outputting it later on at the first verse.
{
  ...
  if (highest_chapter_number[book] == 1) return;
...
void Usfm2XslFo::output_chapter_number_try_at_first_verse (ustring line, Usfm2XslFoStyle * & fo_block_style)
{
  ...
  if (chapter_number_to_output_at_first_verse == 0) return;
...
// This function collects the running headers.
{
  ...
  if (stylepointer->print_in_left_running_header) {
    if (!book_header_left[book].empty ()) book_header_left[book].append (" ");
    book_header_left[book].append (runningheader);
  }
  if (stylepointer->print_in_right_running_header) {
    if (!book_header_right[book].empty ()) book_header_right[book].append (" ");
...
void Usfm2XslFo::signal_progress ()
{
  if (progresswindow) {
    ...
    if (progresswindow->cancel) cancel = true;
  }
}
...
// and then processes that to output the complete table.
{
  ...
  
  {
    ...
    for (unsigned int i = 0; i < table_styles.size(); i++) {
      if (table_styles[i]->type == u2xtTableElementRow) {
        if (!row.cells.empty()) rows_raw.push_back (row);
        row.cells.clear ();
      } 
  ...
  for (unsigned int r = 0; r < rows_tidy.size(); r++) {
    ...
    if (heading) header_or_body.append ("header");
    else header_or_body.append ("body");
    ...
    for (unsigned int c = 0; c < row.cells.size(); c++) {
      ...
      if (style == NULL) style = marker_get_pointer_to_style (default_style ());
      open_fo_block (style, false);
...
// Outputs a footnote or crossreference.
{
  ...
  if (endmarkerpos == string::npos) return;
        ...
        if (stylepointer) {
                  ...
                  || (stylepointer->type == u2xtCrossreferenceStandardContent)) 
          {
            close_possible_fo_inline (inlinestyle);
                  ...
                  || (stylepointer->type == u2xtFootEndNoteContentWithEndmarker)
                  || (stylepointer->type == u2xtCrossreferenceContent)
                  || (stylepointer->type == u2xtCrossreferenceContentWithEndmarker))
          {
            close_possible_fo_inline (inlinestyle);
            if (is_opener) {
...
*/
{
  ...
  if (notecallers_per_page.empty ()) return;
...
// Looks in the style whether the book id should cause a page break.
{
  ...
  if (!stylepointer->book_starts_new_page) return;
...
void Usfm2XslFo::output_page_break  (Usfm2XslFoStyle * & fo_block_style, Usfm2XslFoStyle * & fo_inline_style, bool oddpage)
{
  ...
  if (do_not_allow_new_page) return;
...
// Dumps the buffered endnotes, if there are any.
{
  ...
  if (buffered_endnote_texts.empty ()) return;
