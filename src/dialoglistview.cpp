...
// If the user selects one out of that, it returns it in "focus".
{
  ...
  dialog_vbox1 = GTK_DIALOG (listviewdialog)->vbox;
  ...
  if (sortlist) sort (list.begin(), list.end());
  ...
  dialog_action_area1 = GTK_DIALOG (listviewdialog)->action_area;
                                   ...
                                   gchar * actiontext2)
{
  ...
  gtk_widget_add_accelerator (listviewdialog, "activate_focus", accel_group,
...
void ListviewDialog::on_switch_page ()
{
  ...
  if (actiontext) gtk_label_set_text (GTK_LABEL (label_action), actiontext);
}
