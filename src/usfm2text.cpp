...
void
Usfm2Text::add_usfm_code (const ustring & line)
// Add some usfm code to the object's usfm input buffer.
{
...
void
Usfm2Text::convert_from_usfm_to_text ()
{
  // Cancel?
  if (cancel)
    return;
...
void
Usfm2Text::set_fonts (const vector < ustring > &fonts_in, double font_size_in)
{
  fonts = fonts_in;
...
void
Usfm2Text::set_line_height (unsigned int line_height_in)
{
...
void
Usfm2Text::set_two_columns ()
{
  two_columns = true;
}

void
Usfm2Text::set_even_page_count ()
...
void
Usfm2Text::add_styles (const vector < Usfm2XslFoStyle > &styles_in)
{
...
void
Usfm2Text::add_style (const ustring & marker_in, Usfm2XslFoStyleType type_in)
{
...
bool
Usfm2Text::unprocessed_usfm_code_available ()
...
void
Usfm2Text::output_text_starting_new_paragraph (ustring & line, Usfm2XslFoStyle * stylepointer, Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style, size_t marker_length, bool keep_with_next_paragraph)
/*
...
Usfm2XslFoStyle *
Usfm2Text::marker_get_pointer_to_style (const ustring & marker)
...
void
Usfm2Text::output_text_fallback (ustring & line)
/*
...
void
Usfm2Text::open_paragraph (Usfm2XslFoStyle * style, bool keep_with_next_paragraph)
{
  // Open the paragraph.
...
void
Usfm2Text::set_paragraph (Usfm2XslFoStyle * style, bool keep_with_next_paragraph)
{
  // Fontsize
  ...
  // Space before
  text2pdf->paragraph_set_space_before ((int) style->spacebefore);

  // Space after
  text2pdf->paragraph_set_space_after ((int) style->spaceafter);

  // Start indent (or left margin).
  text2pdf->paragraph_set_left_margin ((int) style->leftmargin);

  // End indent (or right margin).
  text2pdf->paragraph_set_right_margin ((int) style->rightmargin);

  // Text indent (or first line indent).
  text2pdf->paragraph_set_first_line_indent ((int) style->firstlineindent);
...
void
Usfm2Text::open_inline (Usfm2XslFoStyle * style, Usfm2XslFoStyle * block)
{
  // Variable for general use.
  ustring s;

  // Fontsize
  text2pdf->inline_set_font_size_percentage (style->fontpercentage);
  ...
  if (style->italic == ON)
    ...
    text2pdf->inline_set_italic (t2pmtInherit);
  else if (style->italic == TOGGLE)
    {
      text2pdf->inline_set_italic (t2pmtToggle);
    }
  // Bold
  if (style->bold == ON)
    text2pdf->inline_set_bold (t2pmtOn);
  else if (style->bold == OFF)
    text2pdf->inline_set_bold (t2pmtOff);
  else if (style->bold == INHERIT)
    text2pdf->inline_set_bold (t2pmtInherit);
  else if (style->bold == TOGGLE)
    {
      text2pdf->inline_set_bold (t2pmtToggle);
    }
  // Underline
  s.clear ();
  if (style->underline == ON)
    text2pdf->inline_set_underline (t2pmtOn);
  else if (style->underline == OFF)
    text2pdf->inline_set_underline (t2pmtOff);
  else if (style->underline == INHERIT)
    text2pdf->inline_set_underline (t2pmtInherit);
  else if (style->underline == TOGGLE)
    {
      text2pdf->inline_set_underline (t2pmtToggle);
    }
  // Smallcaps
  if (style->smallcaps == ON)
    text2pdf->inline_set_small_caps (t2pmtOn);
  else if (style->smallcaps == OFF)
    text2pdf->inline_set_small_caps (t2pmtOff);
  else if (style->smallcaps == INHERIT)
    text2pdf->inline_set_small_caps (t2pmtInherit);
  else if (style->smallcaps == TOGGLE)
    {
      text2pdf->inline_set_small_caps (t2pmtToggle);
    }
  // Superscript.
  ...
  if (style->color)
    {
      text2pdf->inline_set_colour (style->color);
    }
}

void
Usfm2Text::output_chapter_number_try_normal (ustring & line, Usfm2XslFoStyle * stylepointer, Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style, size_t marker_length, unsigned int book)
...
void
Usfm2Text::output_chapter_number_try_at_first_verse (ustring line, Usfm2XslFoStyle * &fo_block_style)
{
...
void
Usfm2Text::close_possible_inline (Usfm2XslFoStyle * &style)
...
void
Usfm2Text::output_running_header (ustring & line, Usfm2XslFoStyle * stylepointer, size_t marker_length, unsigned int book)
...
bool
Usfm2Text::signal_progress_timeout (gpointer user_data)
{
  ((Usfm2Text *) user_data)->signal_progress ();
  return true;
}

void
Usfm2Text::signal_progress ()
{
  if (progresswindow)
    {
...
bool
Usfm2Text::output_verse_number (Usfm2XslFoStyle * stylepointer, Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style, size_t marker_length)
...
void
Usfm2Text::output_text_character_style (ustring & line, Usfm2XslFoStyle * stylepointer, Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style, size_t marker_length, bool is_opener)
/*
...
void
Usfm2Text::output_text_table (ustring & line, Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style, size_t marker_length)
...
void
Usfm2Text::destroy_note_callers ()
// This function goes through all available styles, and then creates
// the note caller objects from them.
// Or it destroys them.
{
...
void
Usfm2Text::output_text_note (ustring & line, Usfm2XslFoStyle * stylepointer, Usfm2XslFoStyle * &fo_block_style, size_t marker_length, bool is_opener)
...
Usfm2XslFoStyle *
Usfm2Text::get_default_paragraph_style_for_note (Usfm2XslFoStyle * notestyle)
...
void
Usfm2Text::note_callers_new_book ()
...
void
Usfm2Text::note_callers_new_chapter ()
...
void
Usfm2Text::add_print_portion (unsigned int book_in, vector < unsigned int >chapters_from_in, const vector < ustring > &verses_from_in, vector < unsigned int >chapters_to_in, const vector < ustring > &verses_to_in)
...
void
Usfm2Text::set_new_book (unsigned int book_in)
{
  book = book_in;
  set_new_chapter (0);
...
void
Usfm2Text::set_new_chapter (unsigned int chapter_in)
{
  chapter = chapter_in;
  unsigned int left = 0;
...
void
Usfm2Text::set_new_verse (const ustring & verse_in)
{
  verse = verse_in;
...
void
Usfm2Text::output_id_page_break (Usfm2XslFoStyle * stylepointer, Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style)
...
void
Usfm2Text::output_other_page_break (ustring & line, Usfm2XslFoStyle * stylepointer, Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style, size_t marker_length)
...
void
Usfm2Text::output_page_break (Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style, bool oddpage)
// Outputs a page break.
{
...
void
Usfm2Text::output_picture (ustring & line, Usfm2XslFoStyle * stylepointer, Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style, size_t marker_length)
// Prints a picture.
{
...
void
Usfm2Text::output_elastic (ustring & line, Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style, size_t marker_length)
...
void
Usfm2Text::toc_collect_text (ustring & line, size_t marker_length, bool longtext, unsigned int book)
...
void
Usfm2Text::toc_insert_referent (ustring & line, Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style, size_t marker_length)
...
void
Usfm2Text::toc_insert_body (ustring & line, Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style, size_t marker_length)
...
void
Usfm2Text::dump_endnotes (Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style)
...
void
Usfm2Text::buffer_endnote (ustring & line, Usfm2XslFoStyle * stylepointer, size_t marker_length, bool is_opener)
...
void
Usfm2Text::output_text_insertion_deletion (ustring & line, Usfm2XslFoStyle * stylepointer, Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style, size_t marker_length, bool is_opener)
...
void
Usfm2Text::output_spacing_paragraph (ustring & line, Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style, size_t marker_length, bool is_opener)
{
  // Erase the marker from the line.
  line.erase (0, marker_length);
...
void
Usfm2Text::output_keep_on_page (ustring & line, Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style, size_t marker_length, bool is_opener)
{
  // Erase the marker from the line.
  line.erase (0, marker_length);
...
gchar *
Usfm2Text::font_family_size_line_height_style ()
{
  return (gchar *) "_font_family_size_line_height_";
}

void
Usfm2Text::output_font_family_size_line_height (ustring & line, Usfm2XslFoStyle * &fo_block_style, Usfm2XslFoStyle * &fo_inline_style, size_t marker_length, bool is_opener)
...
void
Usfm2Text::optionally_add_full_references (ustring & line, Usfm2XslFoStyle * stylepointer)
{
