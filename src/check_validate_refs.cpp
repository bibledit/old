...
// Do the actual check of one verse.
{
  ...
  for (unsigned int i = 0; i < references.size(); i++) {
    ...
    if (!chapters.empty()) highest_chapter = chapters[chapters.size() - 1];
    if (refscanner.references[i].chapter > highest_chapter) reference_fits = false;
    ...
    if (this_verse > last_verse) reference_fits = false;
    if (!reference_fits) {
