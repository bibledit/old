...
KeytermPreferencesDialog::KeytermPreferencesDialog(int dummy)
{
  my_always_ok = false;

  entrydialog = gtk_dialog_new();
  gtk_window_set_title(GTK_WINDOW(entrydialog), "");
  ...
  new InDialogHelp(entrydialog, NULL, NULL);
...
KeytermPreferencesDialog::~KeytermPreferencesDialog()
{
  gtk_widget_destroy(entrydialog);
}

int KeytermPreferencesDialog::run()
{
  return gtk_dialog_run(GTK_DIALOG(entrydialog));
}

void KeytermPreferencesDialog::on_okbutton_clicked(GtkButton * button, gpointer user_data)
{
  ((KeytermPreferencesDialog *) user_data)->on_okbutton();
}

void KeytermPreferencesDialog::on_okbutton()
{
  entered_value = gtk_entry_get_text(GTK_ENTRY(entry));
}

void KeytermPreferencesDialog::on_entry_changed(GtkEditable * editable, gpointer user_data)
{
  ((KeytermPreferencesDialog *) user_data)->on_entry();
}

void KeytermPreferencesDialog::on_entry()
...
void KeytermPreferencesDialog::text_invisible()
// This function makes the text in the entry invisible.
{
  gtk_entry_set_visibility(GTK_ENTRY(entry), false);
}

void KeytermPreferencesDialog::always_ok()
{
  my_always_ok = true;
  on_entry();
}
