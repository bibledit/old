    ...
    for (unsigned int ch = 0; ch < chapters.size(); ch++) {
      ...
      if (g_file_get_contents (chapterfilename.c_str(), &quickcontent, NULL, NULL)) {
        ...
        if (casesensitive) {
          ...
          if (casefoldedcontent) g_free (casefoldedcontent);          
        }
        if (foundlocation) wordfound = true;
      }
      if (quickcontent) g_free (quickcontent);
      if (wordfound) {
        ReadText rt (chapterfilename, true, false);
        CategorizeChapterVerse ccv (rt.lines);
        for (unsigned int i = 0; i < ccv.verse.size(); i++) {
          if (!casesensitive) ccv.line[i] = ccv.line[i].casefold();
...
void search_load_references (vector<Reference>& searchresults, 
