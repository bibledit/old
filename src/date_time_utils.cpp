...
// Special code is used that works not only on Linux but also on BSD.
{
  ...
  if (g_date_is_leap_year (local_year + 1900)) local_time++;
  ...
  if (g_date_is_leap_year (gm_year + 1900)) gm_time++;
  gm_time += gm_day;
  gm_time *= 24;
  gm_time += gm_hour;
  gm_time *= 60;
...
// Returns the seconds in human readable format.
{
  ...
  if (uhour.length() == 1) uhour.insert (0, "0");
  uminute = convert_to_string (minute);
  if (uminute.length() == 1) uminute.insert (0, "0");
  usecond = convert_to_string (second);
  if (usecond.length() == 1) usecond.insert (0, "0");
