...
ImportRawTextDialog::ImportRawTextDialog (int dummy)
{
  ...
  checkbutton_approve = gtk_check_button_new_with_mnemonic ("Approve text");
  gtk_widget_show (checkbutton_approve);
  gtk_box_pack_start (GTK_BOX (vbox_controls), checkbutton_approve, FALSE, FALSE, 0);

  shortcuts.button (checkbutton_approve);
  */
  ...
  g_signal_connect ((gpointer) button_discover_markup, "clicked",
                    ...
                    gpointer (this));
  /*                    
  g_signal_connect ((gpointer) checkbutton_approve, "toggled",
                    G_CALLBACK (on_checkbutton_approve_toggled),
                    gpointer (this));
  */
...
{
  if (programmatically_setting_text) return;
  gui ();
}
...
void ImportRawTextDialog::on_combobox_chapter ()
{
  ...
  if (!ch.empty ()) chapter = convert_to_int (ch);
  gui ();
}
...
// Discovers the USFM markup of the text.
{
  ...
  if (discoveries_passed) gui ();
}


void ImportRawTextDialog::on_checkbutton_approve_toggled (GtkToggleButton *togglebutton, gpointer user_data)
{
  ((ImportRawTextDialog *) user_data)->on_checkbutton_approve ();
}


void ImportRawTextDialog::on_checkbutton_approve ()
{
  gui ();
}
  ...
  if (checks_passed) {
    ...
    for (unsigned int i = 0; i < required_verses.size (); i++) {
      if (actual_verses_set.find (required_verses[i]) == actual_verses_set.end ()) {
        if (!missing_verses.empty ()) missing_verses.append (", ");
    ...
    if (checks_passed) {
      ...
      for (unsigned int i = 0; i < actual_verses.size (); i++) {
        if (required_verses_set.find (actual_verses[i]) == required_verses_set.end ()) {
          if (!extra_verses.empty ()) extra_verses.append (", ");
    ...
    if (checks_passed) {
      ...
      for (unsigned int i = 0; i < actual_verses.size (); i++) {
        if (double_set.find (actual_verses[i]) != double_set.end ()) {
          if (!double_verses.empty ()) double_verses.append (", ");
    ...
    if (checks_passed) {
      ...
      for (unsigned int i = 0; i < non_line_starters.size (); i++) {
        if (!nostart.empty ()) nostart.append (", ");
