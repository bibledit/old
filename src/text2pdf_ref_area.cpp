...
// Print the data.
  {
    ...
    for (unsigned int i = 0; i < note_layout_containers.size(); i++)
      {
        ...
        layout_container->rectangle.y += rectangle.y + rectangle.height
            - notes_height;
    ...
    for (unsigned int i = 0; i < table_of_contents_layout_containers.size(); i++)
      {
        T2PLayoutContainer * layout_container =
            table_of_contents_layout_containers[i];
        layout_container->rectangle.x += rectangle.x;
...
void T2PReferenceArea::print(unsigned int page_number, bool print_date,
    const ustring& left_running_header, const ustring& right_running_header,
    bool suppress_header, unsigned int left_first_chapter,
    unsigned int left_last_chapter, unsigned int right_first_chapter,
    unsigned int right_last_chapter)
// Print the page number, the date, the running header.
  {
    ...
    if (!(page_number % 2))
      {
        // Even.
        page_number_layout_container.rectangle.x += rectangle.width
            - page_number_layout_container.rectangle.width;
      }
    ...
    if (print_date)
      {
        string s(date_time_julian_human_readable(
            date_time_julian_day_get_current(), false));
        date_layout_container.layout_text(NULL, 0, s);
      }
    if ((page_number % 2))
      {
        // Odd.
        date_layout_container.rectangle.x
            += page_number_layout_container.rectangle.width
                + millimeters_to_pango_units(5);
      }
    else
      {
        // Even.
        date_layout_container.rectangle.x += rectangle.width
            - date_layout_container.rectangle.width
            - millimeters_to_pango_units(5)
            - page_number_layout_container.rectangle.width;
      }
    ...
    string header(produce_running_header(running_header, suppress_header,
    ...
    if (!header.empty())
      {
        ...
        if ((page_number % 2))
          {
            // Odd page.
            running_header_layout_container.rectangle.x += rectangle.width
                - running_header_layout_container.rectangle.width;
          }
        else
          {
            // Even page.
...
void T2PReferenceArea::fit_blocks(deque <T2PBlock *>& input_blocks,
    int column_spacing_pango_units_in)
    ...
    while (!input_blocks.empty())
      {
        ...
        if (input_blocks[0]->column_count != n_columns
            || new_page_input_block_encountered(input_blocks, false))
          {
...
void T2PReferenceArea::fit_columns(deque <T2PBlock *>& input_blocks,
    int column_count)
/*
    ...
    while (!input_blocks.empty())
      {
        ...
        T2PBigBlock big_block = get_next_big_block_to_be_kept_together(
            input_blocks, column_count);
        ...
        for (unsigned int i = 0; i < big_block.blocks.size(); i++)
          {
            ...
            for (unsigned int i2 = 0; i2 < block->layoutcontainers.size(); i2++)
              {
                T2PLayoutContainer * layout_container =
                    block->layoutcontainers[i2];
                for (unsigned int i3 = 0; i3
                    < layout_container->note_paragraphs.size(); i3++)
                  {
                    T2PInputParagraph * note_paragraph =
                        layout_container->note_paragraphs[i3];
                    while (!note_paragraph->text.empty())
                      {
                        PangoRectangle note_rectangle =
                            get_next_free_note_rectangle();
                        T2PLayoutContainer * note_layout_container =
                            new T2PLayoutContainer (note_rectangle, NULL, cairo);
                        note_layout_container->set_has_note();
                        note_layout_container->layout_text(note_paragraph, 0,
                            note_paragraph->text);
        ...
        int first_column_height = get_column_height(first_column,
            start_stacking_y);
        int last_column_height = get_column_height(last_column,
            start_stacking_y);
        bool columns_too_high = (start_stacking_y + MAX (first_column_height, last_column_height)) > (rectangle.height
            - get_note_height());
        if (columns_too_high)
          {

            // If there's only one block on the page, special handling is needed to avoid an infinite loop.
            bool one_block_on_page = first_column.size() + last_column.size()
                == 1;
            ...
            while (!added_notes.empty())
              {
                T2PLayoutContainer * note_layout_container =
                    note_layout_containers[note_layout_containers.size()-1];
                if (!one_block_on_page)
                ...
                if (one_block_on_page)
                  {
                    columns_too_high = (start_stacking_y + MAX (first_column_height, last_column_height))
                        > (rectangle.height - get_note_height());
                    if (!columns_too_high)
                      break;
                  }
              }
    ...
    for (unsigned int i = 0; i < last_column.size(); i++)
      {
        if (last_column[i].column_count == 1)
          ...
          {
            last_column[i].set_blocks_x(rectangle.width - ((rectangle.width
                - column_spacing_pango_units) / 2));
          }
...
T2PBigBlock T2PReferenceArea::get_next_big_block_to_be_kept_together(
    deque <T2PBlock *>& input_blocks, int column_count)
...
int T2PReferenceArea::get_column_height(deque <T2PBigBlock>& column,
    int reference_y)
...
// Gets the next free rectangle where a note could be laid out.
  {
    ...
    if (!note_layout_containers.empty())
      {
        T2PLayoutContainer * last_container =
            note_layout_containers[note_layout_containers.size()-1];
    ...
    double remaining_millimeters = pango_units_to_millimeters(rectangle.width
        - next_x);
...
void T2PReferenceArea::balance_columns(deque <T2PBigBlock>& first_column,
    deque <T2PBigBlock>& last_column)
...
// or from the last one to the first, depending on the needs.
  {
    int last_higher_total_height =
        balance_last_column_higher_than_or_equal_to_first_column(first_column,
            last_column);
    int first_higher_total_height =
        balance_first_column_higher_than_or_equal_to_last_column(first_column,
            last_column);
...
int T2PReferenceArea::balance_last_column_higher_than_or_equal_to_first_column(
    deque <T2PBigBlock>& first_column, deque <T2PBigBlock>& last_column)
...
// Returns the maximum height of both columns.
  {
    ...
    while (last_column_height <= first_column_height && !first_column.empty())
      {
        T2PBigBlock last_big_block_of_first_column =
            first_column[first_column.size()-1];
...
int T2PReferenceArea::balance_first_column_higher_than_or_equal_to_last_column(
    deque <T2PBigBlock>& first_column, deque <T2PBigBlock>& last_column)
...
ustring T2PReferenceArea::produce_running_header(const ustring& header,
    bool suppress, unsigned int first_chapter, unsigned int last_chapter)
...
bool T2PReferenceArea::new_page_input_block_encountered(
    deque <T2PBlock *>& input_blocks, bool set_flag)
...
void T2PReferenceArea::get_referents(map <ustring, ustring>& referents,
...
void T2PReferenceArea::match_and_expand_references_and_referents(
            ...
            if (!referent.empty())
              {
                ...
                T2PLayoutContainer * page_number_layout =
                    new T2PLayoutContainer (toc_rectangle, NULL, cairo);
                table_of_contents_layout_containers.push_back(page_number_layout);
                page_number_layout->layout_text(NULL, 0, referent);
                page_number_layout->rectangle.x = block->rectangle.x
                    + block->maximum_width_pango_units
                    - page_number_layout->rectangle.width;

                int dot_distance_pango_units = millimeters_to_pango_units(2.5);
                int available_width = block->maximum_width_pango_units
                    - block->rectangle.x - block->rectangle.width
                    - dot_distance_pango_units - dot_distance_pango_units
                    - page_number_layout->rectangle.width;
                int start_x = block->rectangle.x + block->rectangle.width;

                while (available_width > 0)
                  {
                    T2PLayoutContainer * dotted_line_layout =
                        new T2PLayoutContainer (toc_rectangle, NULL, cairo);
                    table_of_contents_layout_containers.push_back(dotted_line_layout);
                    string dot(".");
                    dotted_line_layout->layout_text(NULL, 0, dot);
                    dotted_line_layout->rectangle.x = start_x
                        + dot_distance_pango_units;
                    start_x += (dotted_line_layout->rectangle.width
                        + dot_distance_pango_units);
                    available_width -= (dotted_line_layout->rectangle.width
                        + dot_distance_pango_units);
                  }
              }
          }
