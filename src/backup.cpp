...
void backup_make_flexible ()
{
  extern Settings * settings;
  ProjectConfiguration * projectconfig = settings->projectconfig (settings->genconfig.project_get());
  backup_make (settings->genconfig.project_get(),
               projectconfig->backup_full_get(),
               projectconfig->backup_incremental_last_time_get());
...
void backup_make (const ustring& project, bool full, int timefrom)
{
  ...
  ustring currentsuffix = "-"
                        + convert_to_string (currentyear) + "-" 
                        + convert_to_string (currentmonth) + "-"
                        + convert_to_string (currentday) + "-"
                        + convert_to_string (currenthour) + "-"
                        + convert_to_string (currentminute) + "-"
                        + convert_to_string (currentsecond);  
  ustring filename;
  if (full) {
    ...
    ustring fromsuffix = "-from-"
                       + convert_to_string (fromyear) + "-" 
                       + convert_to_string (frommonth) + "-"
                       + convert_to_string (fromday) + "-"
                       + convert_to_string (fromhour) + "-"
                       + convert_to_string (fromminute) + "-"
                       + convert_to_string (fromsecond);  
