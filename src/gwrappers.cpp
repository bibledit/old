...
void GwSpawn::run ()
{
  ...
  // Spawn process.
  if (myasync) {
    result = g_spawn_async_with_pipes (workingdirectory, argv, NULL, 
                                       (GSpawnFlags) flags, 
                                       NULL, NULL, &pid, 
                                       standard_input_filedescriptor_pointer, 
                                       standard_output_filedescriptor_pointer, 
                                       standard_error_filedescriptor_pointer,
                                       NULL);
  ...
  if (myprogress || standard_input_filedescriptor) {
    ...
    if (myprogress) progresswindow = new ProgressWindow (mytext, myallowcancel);
...
// todo        unix_kill (pid);
  ...
  if (!mydevnull) {
    ...
    if (myread) {
      ...
      if (standard_output) gw_spawn_write (1, standard_output);
      if (standard_error) gw_spawn_write (2, standard_error);
    }
    // Free data.
    if (standard_output) g_free (standard_output);
    if (standard_error) g_free (standard_error);  
