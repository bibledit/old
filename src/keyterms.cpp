...
void keyterms_import_textfile_flush(sqlite3 *db, unsigned int category_id,
    ustring& keyterm, unsigned int& level, unsigned int& previous_level,
    vector<unsigned int>& parents, vector<ustring>& comments,
    ...
    if (!keyterm.empty())
      {
        ...
        for (unsigned int i = previous_level + 1; i < level; i++)
          {
            sql = g_strdup_printf(
                "insert into keyterms values ('%s', '%s', %d, %d, %d, '%s');",
                "missing", "", category_id, i, parents[i - 1], "");
            rc = sqlite3_exec(db, sql, NULL, NULL, &error);
            g_free(sql);
            if (rc)
        ...
        sql = g_strdup_printf(
            "insert into keyterms values ('%s', '%s', %d, %d, %d, '%s');",
            keyterm.c_str(), keyterm.casefold().c_str(), category_id, level, parent, comment.c_str());
        rc = sqlite3_exec(db, sql, NULL, NULL, &error);
        g_free(sql);
        if (rc)
          throw runtime_error (sqlite3_errmsg(db));
        ...
        sql = g_strdup_printf("select ROWID from keyterms where name = '%s';",
            keyterm.c_str());
        ...
        for (unsigned int i = 0; i < references.size(); i++)
          {
            sql = g_strdup_printf(
                "insert into refs values (%d, %d, %d, '%s');", keyterm_id,
                references[i].book, references[i].chapter,
                references[i].verse.c_str());
            rc = sqlite3_exec(db, sql, NULL, NULL, &error);
            g_free(sql);
            if (rc)
              throw runtime_error (sqlite3_errmsg(db));
        ...
        for (unsigned int i = 0; i < related.size(); i++)
          {
            ...
            sql = g_strdup_printf("insert into related values (%d, '%s');",
                keyterm_id, rel.c_str());
            rc = sqlite3_exec(db, sql, NULL, NULL, &error);
            g_free(sql);
            if (rc)
              throw runtime_error (sqlite3_errmsg(db));
    ...
    try
      {
        ...
        for (unsigned int i = 0; i < 10; i++) parents.push_back (1);
            ...
            try
              {
                ...
                if (line.find ("\\en ") != string::npos) process_lines = true;
                if (line.find ("en AAA") != string::npos) process_lines = false;
                if (!process_lines) continue;
...
void keyterms_import_ktbh_txt_comments(ustring key, const ustring& label,
    ustring line, vector<ustring>& comments)
  {
    key.insert(0, "\\");
    key.append(" ");
...
void keyterms_import_ktbh_txt_references(ustring line,
    vector<Reference>& references)
/*
    ...
    for (unsigned int i = 0; i < parse.words.size(); i++)
      {
        ...
        *digit5 -= 32;
        Reference reference(*digit1, digit2 + *digit3, convert_to_string(digit4
            + *digit5));
        references.push_back(reference);
        g_free(digit1);
        g_free(digit3);
    ...
    try
      {
        ...
        char * sql;
        SqliteReader reader (0);
        sql = g_strdup_printf ("select rendering, wholeword, casesensitive from renderings where keyword = '%s' and collection = '%s';",
            double_apostrophy (keyterm).c_str(), double_apostrophy (collection).c_str());
        rc = sqlite3_exec(db, sql, reader.callback, &reader, &error);
        g_free (sql);
        if (rc) throw runtime_error (sqlite3_errmsg(db));
...
void keyterms_store_renderings(const ustring& project, const ustring& keyterm,
    const ustring& collection, const vector<ustring>& renderings,
    ...
    try
      {
        ...
        sqlite3_busy_timeout (db, 1000);
        char * sql;
        sql = g_strdup_printf ("delete from renderings where keyword = '%s' and collection = '%s';",
            double_apostrophy (keyterm).c_str(), double_apostrophy (collection).c_str());
        rc = sqlite3_exec (db, sql, NULL, NULL, &error);
        g_free (sql);
        for (unsigned int i = 0; i < renderings.size(); i++)
          {
            sql = g_strdup_printf ("insert into renderings values ('%s', '%s', '%s', %d, %d);",
                double_apostrophy (keyterm).c_str(),
                double_apostrophy (collection).c_str(),
                double_apostrophy (renderings[i]).c_str(),
                wholewords[i],
                casesensitives[i]);
            rc = sqlite3_exec (db, sql, NULL, NULL, &error);
            g_free (sql);
...
vector <int> keyterms_get_terms_in_verse(const Reference& reference)
  {
    ...
    char
        * sql =
            g_strdup_printf(
                "select keyword from refs where book = %d and chapter = %d and verse = '%s';",
