...
MyChecksDialog::MyChecksDialog (GtkListStore * liststore, GtkWidget * treeview, GtkTreeViewColumn * treecolumn)
{
  ...
  dialog_vbox1 = GTK_DIALOG (mychecksdialog)->vbox;
  ...
  dialog_action_area1 = GTK_DIALOG (mychecksdialog)->action_area;
  gtk_widget_show (dialog_action_area1);
  gtk_button_box_set_layout (GTK_BUTTON_BOX (dialog_action_area1), GTK_BUTTONBOX_END);

  new InDialogHelp (mychecksdialog, &shortcuts, NULL);
...
void MyChecksDialog::on_okbutton ()
{
  ...
  if (bitpattern_take (pattern)) if (keep_going) keep_going = scripture_checks_chapters_verses (NULL, NULL, NULL, &results);
  if (bitpattern_take (pattern)) if (keep_going) keep_going = scripture_checks_validate_usfms (NULL, NULL, NULL, &results);
  if (bitpattern_take (pattern)) if (keep_going) keep_going = scripture_checks_count_usfms (false);
  if (bitpattern_take (pattern)) if (keep_going) keep_going = scripture_checks_compare_usfms (NULL, NULL, NULL, &results);
  if (bitpattern_take (pattern)) if (keep_going) keep_going = scripture_checks_count_characters (false); 
  if (bitpattern_take (pattern)) if (keep_going) keep_going = scripture_checks_unwanted_patterns (NULL, NULL, NULL, &results); 
  if (bitpattern_take (pattern)) if (keep_going) keep_going = scripture_checks_capitalization (NULL, NULL, NULL, &results); 
  if (bitpattern_take (pattern)) if (keep_going) keep_going = scripture_checks_repetition (NULL, NULL, NULL, &results); 
  if (bitpattern_take (pattern)) if (keep_going) keep_going = scripture_checks_unwanted_words (NULL, NULL, NULL, &results); 
  if (bitpattern_take (pattern)) if (keep_going) keep_going = scripture_checks_word_inventory (false); 
  if (bitpattern_take (pattern)) if (keep_going) keep_going = scripture_checks_matching_pairs (NULL, NULL, NULL, &results); 
