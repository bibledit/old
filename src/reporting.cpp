...
{
  struct { char * task; double duration; } default_values [] = 
  {
    { "First Draft",                 2   },
    { "Self Check",                  0.4 },
    { "Team Check",                  0.4 },
...
// from the children.
{
  ...
  for (unsigned int i = 0; i < taskcount; i++) {
    ...
    if (inconsistent) {
      ...
      if (initialstatus) parent->tasks_done[i] = 1;
      else parent->tasks_done[i] = 0;
    }
  }
}
...
ProjectStatusBook::ProjectStatusBook (const ustring& project_in, unsigned int book_in, vector <ustring> * alltasks_in)
{
  ...
  for (unsigned int i = 0; i < chps.size (); i++) {
    ...
    if (highestverse == 1) highestverse = 0;
...
ProjectStatus::ProjectStatus (const ustring& project_in, vector <ustring> * alltasks_in, bool gui)
{
  ...
  if (gui) progresswindow->set_iterate (0, 1, bks.size ());
...
// This produces a status report.
{
  ...
  if (tasks_book) {
    ...
    for (unsigned int i = 0; i < column_headers.size (); i++) {
      if (i == 0) centers.push_back (false);
      else centers.push_back (true);
    }
    htmlwriter.table ("Tasks per book", column_headers, cell_texts, "", &centers, -1);
    htmlwriter.paragraph ("");
  }
  ...
  if (tasks_chapter) {
    for (unsigned int i = 0; i < projectstatus.books.size (); i++) {
      ...
      for (unsigned int i2 = 0; i2 < column_headers.size (); i2++) {
        if (i2 == 0) centers.push_back (false);
        else centers.push_back (true);
      }
