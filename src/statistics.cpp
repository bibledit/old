...
ustring statistics_database (const ustring& project, const gchar * suffix)
{
  ...
  if (suffix) filename.append (suffix);
  // Current suffix.
  else filename.append ("sql1"); 
  return filename;
}
...
// Upgrades if needed.
{
  ...
  if (gui) progresswindow = new ProgressWindow (project + ": upgrading statistics", false);
        ...
        for (unsigned int ch = 0; ch < chapters.size(); ch++) {
          ...
          for (unsigned int vs = 0; vs < verses.size(); vs++) {
            sql = g_strdup_printf ("insert into edits values (%d, %d, '%s', %d)", 
                                   books[bk], chapters[ch], verses[vs].c_str(), seconds);
            rc = sqlite3_exec (db, sql, NULL, NULL, NULL);
            g_free (sql);
            if (rc) throw runtime_error (sqlite3_errmsg(db));
          }
                                      ...
                                      CategorizeChapterVerse& ccv, unsigned int first, unsigned int last)
{
  ...
  if (book_exists) current_verses = project_get_verses (project, book, chapter);
  ...
  if (!edits.empty()) {
    ...
    try
    {
      ...
      for (unsigned int i = 0; i < edits.size(); i++) {
        sql = g_strdup_printf ("insert into edits values (%d, %d, '%s', %d)", 
                               edits[i].book, edits[i].chapter, edits[i].verse.c_str(), seconds);
        rc = sqlite3_exec (db, sql, NULL, NULL, NULL);
        g_free (sql);
        if (rc) throw runtime_error (sqlite3_errmsg(db));
      }
