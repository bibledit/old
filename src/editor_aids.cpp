...
{
  switch (type) {
    case eudInsertText:   return "InsertText";
    case eudDeleteText:   return "DeleteText";
    case eudApplyTag:     return "ApplyTag";
    case eudRemoveTag:    return "RemoveTag";
  }
  return "";
}
...
// a footnote.
{
  switch (type) {
    ...
    case stFootEndNote:
    {
      if (subtype == fentFootnote) return true;
      break;
    }
    case stCrossreference:
    case stPeripheral:
...
// an endnote.
{
  switch (type) {
    ...
    case stFootEndNote:
    {
      if (subtype == fentEndnote) return true;
      break;
    }
    case stCrossreference:
    case stPeripheral:
...
// a crossreference.
{
  switch (type) {
    ...
    case stCrossreference:
    {
      if (subtype == ctCrossreference) return true;
      break;
    }
    case stPeripheral:
    case stPicture:
...
// Erases the character before the text insertion point if that is a space.
{
  ...
  if (!text_available) return;
  gunichar last_character = gtk_text_iter_get_char (&iter);
  if (!g_unichar_isspace (last_character)) return;
...
{
  if (gtk_text_buffer_get_modified (mainbuffer)) return true;
  for (unsigned int i = 0; i < editornotes.size(); i++) {
    if (gtk_text_buffer_get_modified (editornotes[i].textbuffer)) return true;
  }
  for (unsigned int i = 0; i < editortables.size (); i++) {
    for (unsigned int row = 0; row < editortables[i].textbuffers.size (); row++) {
      for (unsigned int column = 0; column < editortables[i].textbuffers[row].size (); column++) {
        GtkTextBuffer * textbuffer = table_cell_get_buffer (editortables[i], row, column);
        if (gtk_text_buffer_get_modified (textbuffer)) return true;    
      }
    }
  }
  return false;
  ...
  if (offset == 0) {
    ...
    for (unsigned int i = 1; i < editornotes.size (); i++) {
      ...
      if (editornotes[i].textview) {
        ...
        if (function) gtk_text_iter_backward_char (&iter);
        iterfound = true;
      }
    }
  }
...
// Else applies the "unknown" style.
{
  ...
  if (tag) gtk_text_buffer_apply_tag_by_name (buffer, name.c_str (), start, end);
  else gtk_text_buffer_apply_tag_by_name (buffer, unknown_style (), start, end);  
}
