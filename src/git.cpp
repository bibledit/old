...
// Does initial checks on the local git repositories.
{
  ...
  for (unsigned int i = 0; i < projects.size(); i++) {
    ...
    if (projectconfig->editable_get ()) {
      git_initial_check_project (projects[i]);
    }
  }
}
...
// Upgrades if needed.
{
  ...
  git_schedule (gttInitializeProject, project, 0, 0, "");
}
...
void git_erase_task_done ()
{
  ...
  if (!gittasks.empty()) {
    ...
    for (unsigned int i = 0; i < gittasks.size(); i++) {
      if (   task.task != gittasks[i].task
          || task.project != gittasks[i].project
          || task.book != gittasks[i].book
          || task.chapter != gittasks[i].chapter)
...
void git_erase_task (GitTaskType task, const ustring& project, unsigned int book, unsigned int chapter)
{
  vector <GitTask> newtasks;
  for (unsigned int i = 0; i < gittasks.size(); i++) {
    if (   task    != gittasks[i].task
        || project != gittasks[i].project
        || book    != gittasks[i].book
        || chapter != gittasks[i].chapter)
      newtasks.push_back (gittasks[i]);  
  }
  gittasks = newtasks;
}
  ...
  */
  ...
  for (unsigned int i = 0; i < rt.lines.size(); i++) {
    ...
    if (!minemarker && !separatormarker && !servermarker) {
      if (withinmine) {
        if (conflicthandling == gchtTakeMe) takeit = true;
      } else if (withinserver) {
        if (conflicthandling == gchtTakeServer) takeit = true;
      } else {
        takeit = true;
      }   
    }
