...
We combine the two sources into one on import, and store a database format as richtext as:
( 222 he created) the ( 3433 heavens)
The text that is covered by the Strong's number is put together with the brackets.
So searching for the strong's number goes like looking for a space, then a number, then a space.
This way we can store two Strong's numbers as well, as may happen at times.
Or do it like:
[S222 he created] the (...
Point is to use a special system so that also the morphology fits into the brackets, e.g. as M222.
We look whether the text can be combined into one, if not we use a slash, e.g. "heaven/heavens".
