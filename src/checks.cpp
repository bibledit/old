...
void DisplayCheckingResults::word_inventory (const vector<ustring>& words, const vector<unsigned int>& count,
...
void DisplayCheckingResults::add_table_cell (const ustring& text, bool bold, bool right_aligned, bool translate_entities)
{
  ...
  if (bold) 
    line.append ("<b>");
  ustring text2 (text);
  if (translate_entities) xml_handle_entities (text2, NULL);
  line.append (text2);
  if (bold) 
    line.append ("</b>");
  line.append ("</td>");
