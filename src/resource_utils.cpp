...
vector <ustring> resource_get_resources (vector <ustring>& filenames, bool list_deleted_ones)
{
  ...
  for (unsigned int i = 0; i < rf.files.size (); i++) {
    ...
    ustring title = resource_get_title (filename);
    if (unwanted_ones.find (title) != unwanted_ones.end ()) continue;
    filenames.push_back (filename);
...
gchar * resource_type_to_text (ResourceType type)
{
  switch (type) {
    ...
    case rtIndexFileOnFlyForEachVerseWithDifferentAnchors:
      return "Produce a different index file on the fly for each verse, with different anchors";
