...
GitSetupDialog::GitSetupDialog (int dummy)
{
  ...
  spawn.run ();
  ustring versiontext;
  if (!spawn.standardout.empty ()) versiontext = spawn.standardout[0];
...
void GitSetupDialog::set_gui ()
{
  ...
  if (action_sync) gtk_label_set_text (GTK_LABEL (label_pull_info), "Pull changes check");
  if (action_copy) gtk_label_set_text (GTK_LABEL (label_pull_info), "Copy project check");
  ...
  if (pending_tasks_okay) git_command_pause (true);
  
  // If there are still pending tasks, bail out.
  if (!pending_tasks_okay) return; 
  ...
  if (!action_copy && !action_sync) return;  
