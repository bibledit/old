/* $Id: ustring.cc,v 1.3 2004/05/26 20:33:03 daniel Exp $ */
                                      ...
                                      bool find_not_of)
{
  ...
  const GlibScopedPtr<gunichar> ucs4_match
...
ustring ustring::normalize(GlibNormalizeMode mode) const
{
  const GlibScopedPtr<char> buf (g_utf8_normalize(string_.data(), string_.size(),
...
{
  const GlibScopedPtr<char> buf (g_utf8_strup(string_.data(), string_.size()));
  return ustring(buf.get());
}

ustring ustring::lowercase() const
{
  const GlibScopedPtr<char> buf (g_utf8_strdown(string_.data(), string_.size()));
  return ustring(buf.get());
}

ustring ustring::casefold() const
{
  const GlibScopedPtr<char> buf (g_utf8_casefold(string_.data(), string_.size()));
  return ustring(buf.get());
}

std::string ustring::collate_key() const
{
  const GlibScopedPtr<char> buf (g_utf8_collate_key(string_.data(), string_.size()));
...
std::string ustring::casefold_collate_key() const
{
  ...
  g_free(casefold_buf);
  return std::string(GlibScopedPtr<char>(key_buf).get());
}


/**** ustring::SequenceToString **************************************/

ustring::SequenceToString<ustring::iterator,gunichar>
  ::SequenceToString(ustring::iterator pbegin, ustring::iterator pend)
:
  std::string(pbegin.base(), pend.base())
{}

ustring::SequenceToString<ustring::const_iterator,gunichar>
  ::SequenceToString(ustring::const_iterator pbegin, ustring::const_iterator pend)
:
  std::string(pbegin.base(), pend.base())
{}


/**** ustring -- stream I/O operators ********************************/

std::istream& operator>>(std::istream& is, ustring& utf8_string)
{
  std::string locale_string;
  is >> locale_string;
  utf8_string = g_locale_to_utf8(locale_string.c_str(), -1, NULL, NULL, NULL);
  return is;
}

std::ostream& operator<<(std::ostream& os, const ustring& utf8_string)
{
  os << g_locale_from_utf8(utf8_string.c_str(), -1, NULL, NULL, NULL);
  return os;
}
