    ...
    switch (stage)
      ...
      case STAGE_COMMUNICATE:
      {
        ...
        if (program_is_running (settings->genconfig.bibleworks_executable_get ()) || settings->genconfig.outpost_networked_get ())
          send_scheduled (bibleworks_reference_set_value);
        send_scheduled (santafefocus_reference_set_value);
        send_scheduled (santafefocus_word_set_value);
        /*
        Later: 
...
void WindowsOutpost::disconnect ()
{
  if (sock) {
    ...
    extern Settings * settings;
    if (settings->genconfig.outpost_networked_get ()) send_line ("show");
    else send_line ("exit");
    close (sock);
  }
  sock = 0;
...
void WindowsOutpost::send_scheduled (ustring& value)
// Carries out a scheduled sending operation.
{
  // If we've a value, send it out.
  if (!value.empty()) {
    send_line (value);
    // If still connected, that is, if sending succeeded, clear value.
    if (connected)
      value.clear();
  }
}
