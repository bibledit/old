...
INSTALLATION:
  ...
  - rename phpliteadmin.config.sample into phpliteadmin.config
  - do not change the settings in phpliteadmin but in phpliteadmin.config
  See https://code.google.com/p/phpliteadmin/wiki/Configuration for details.

	1. Open phpliteadmin.config (or phpliteadmin) in a text editor.
	...
	3. If you want to specify your databases manually, set the value of the $directory variable as false and modify the $databases array to hold the databases you would like to manage.
		...
		- The path field is the file path of the database relative to where phpliteadmin will be located on the server. For example, if phpliteadmin is located at "databases/manager/phpliteadmin" and you want to manage "databases/yourdatabase.sqlite", the path value would be "../yourdatabase.sqlite".
	...
	6. Save and upload phpliteadmin to your web server.
	
	7. Open a web browser and navigate to the uploaded phpliteadmin file. You will be prompted to enter a password. Use the same password you set in step 4.
...
INSTALLING A THEME:
	...
	3. Upload phpliteadmin.css from the theme's directory alongside phpliteadmin.
