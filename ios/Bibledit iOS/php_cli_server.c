...
sapi_module_struct cli_server_sapi_module = {
	"ios-server",					/* name */
	"PHP for iOS HTTP server",		/* pretty name */
...
int do_cli_server(int argc, char **argv, char * root TSRMLS_DC) 
{
	char *php_optarg = NULL;
	int php_optind = 1;
	extern const opt_struct OPTIONS[];
	php_getopt(argc, argv, OPTIONS, &php_optarg, &php_optind, 0, 2);

	const char *server_bind_address = "localhost:8080";

	const char *document_root = root;
	char document_root_buf[MAXPATHLEN];
	if (document_root) {
		struct stat sb;
		stat (document_root, &sb);
...

	{
        ...
        /*
		printf("PHP %s Development Server started\n"
				"Listening on http://%s\n"
				"Document root is %s\n"
				"Press Ctrl-C to quit.\n",
				PHP_VERSION, server_bind_address, document_root);
        */
	}
